...
def _clean_dict(d: dict) -> dict:
    """JavaScript 64-bit int cheklovi tufayli ID maydonlarini string formatiga o'tkazish."""
    if not isinstance(d, dict):
        return d
    res = {}
    id_fields = {"id", "user_id", "post_id", "comment_id", "news_id", "parent_id", "follower_id", "following_id", "boss_id", "cert_id", "target_id"}
    for k, v in d.items():
...
class _CursorWrap:
    ...
    def fetchone(self):
        ...
        if row is None:
            return None
        return _clean_dict(dict(zip(self._cols(), row)))

    def fetchall(self):
        if not self._cur.description:
            return []
        cols = self._cols()
        return [_clean_dict(dict(zip(cols, row))) for row in self._cur.fetchall()]
...
def post_like(b: LikeReq):
    ...
    if r and int(r["is_like"]) == b.is_like:
...
def comment_like(b: CommentLikeReq):
    ...
    cnt_row = c.execute("SELECT COUNT(*) AS cnt FROM comment_likes WHERE comment_id=?", (b.comment_id,)).fetchone()
...
def follow(b: FollowReq):
    ...
    if not tg: c.close(); return err("Topilmadi!", 404)
    if str(tg["id"]) == str(b.follower_id): c.close(); return err("O'zingizga follow bosolmaysiz!")
...
def news_comment_like(b: NewsCommentLikeReq):
    ...
    cnt_row = c.execute("SELECT COUNT(*) AS cnt FROM news_comment_likes WHERE comment_id=?", (b.comment_id,)).fetchone()
    cnt = int(cnt_row["cnt"]) if cnt_row else 0
    c.commit(); c.close(); return {"liked": liked, "count": cnt}
