...
DATABASE_URL = os.getenv(
    "DATABASE_URL",
    "postgresql://ilyosbek:CAIsL_qC1EfkDeRKwyN98Q@chalkerdb-19950.jxf.gcp-europe-west3.cockroachlabs.cloud:26257/defaultdb?sslmode=verify-full",
)
...
# ---------------------------------------------------------------------------
# BigInt JS Precision Fix (ID lar string bo'lib boradi, JS yaxlitlab qo'ymaydi)
# ---------------------------------------------------------------------------
def stringify_ids(data: Any) -> Any:
    """CockroachDB generated 64-bit int IDs are too big for JS numbers.
    Convert all ID fields to string before sending JSON response."""
    if isinstance(data, list):
        return [stringify_ids(item) for item in data]
    elif isinstance(data, dict):
        new_dict = {}
        for k, v in data.items():
            if k in ("id", "post_id", "comment_id", "news_id", "parent_id", "user_id", "follower_id", "following_id") and v is not None:
                new_dict[k] = str(v)
            else:
                new_dict[k] = stringify_ids(v)
        return new_dict
    return data


def safe_json(data: Any, status_code: int = 200, headers: Optional[dict] = None) -> JSONResponse:
    default_headers = {"Cache-Control": "no-store, no-cache, must-revalidate, max-age=0"}
    if headers:
        default_headers.update(headers)
    return JSONResponse(content=stringify_ids(data), status_code=status_code, headers=default_headers)
...
def err(m, s=400):
    return safe_json({"error": m}, status_code=s)
def urow(c, uid):
...
# Pydantic modellari: String ham, Int ham qabul qila oladi
...
def ping_db():
    try:
        ...
        return safe_json({"error": str(e)}, status_code=503)
...
def check_username(username: str, exclude_id: Optional[Union[int, str]] = None):
    u = clean_u(username)
    if not u:
        return safe_json({"available": False})
    c = db()
    ...
    c.close()
    return safe_json({"available": row is None})
...
def login(username: str = Form(...), password: str = Form(...)):
    ...
    c.close()
    return safe_json({"user": pub(r)}) if r else err("Username yoki parol xato!", 401)
...
def profile(user_id: Union[int, str] = Form(...), fullname: str = Form(""), school_class: str = Form(""),
    ...
    c.commit(); r = urow(c, uid); c.close()
    return safe_json({"user": pub(r)}) if r else err("Topilmadi!", 404)
...
def get_user(username: str, viewer_id: Optional[Union[int, str]] = None):
    ...
    vid = int(viewer_id) if viewer_id is not None else -1
...
def followers_list(username: str, viewer_id: Optional[Union[int, str]] = None):
    ...
    c.close()
    return safe_json([{**dict(r), "is_following": bool(r["is_following"])} for r in rows])
...
def certificates(username: str):
    c = db()
    u = c.execute("SELECT id FROM users WHERE username=%s", (clean_u(username),)).fetchone()
    if not u: c.close(); return safe_json([])
...
def certificate_create(b: CertCreate):
    ...
    c.commit(); c.close(); return safe_json({"success": True})
...
async def upload_image(file: UploadFile = File(...)):
    try:
        ...
        return safe_json({"url": f"{R2_PUBLIC_URL}/{key}"})
    except Exception as e:
...
def post_update(b: PostEdit):
    ...
    if not r or r["user_id"] != uid: c.close(); return err("Ruxsat yo'q!", 403)
...
def post_like(b: LikeReq):
    ...
    if r and r["is_like"] == b.is_like:
        c.execute("DELETE FROM likes WHERE user_id=%s AND post_id=%s", (uid, pid)); liked = False
    elif r:
        c.execute("UPDATE likes SET is_like=%s WHERE user_id=%s AND post_id=%s", (b.is_like, uid, pid)); liked = True
    else:
        c.execute("INSERT INTO likes(user_id,post_id,is_like) VALUES(%s,%s,%s)", (uid, pid, b.is_like)); liked = True
    c.commit(); c.close(); return safe_json({"liked": liked})
...
def comment_like(b: CommentLikeReq):
    ...
    if c.execute("SELECT 1 FROM comment_likes WHERE user_id=%s AND comment_id=%s", (uid, cid)).fetchone():
        c.execute("DELETE FROM comment_likes WHERE user_id=%s AND comment_id=%s", (uid, cid)); liked = False
    else:
        c.execute("INSERT INTO comment_likes(user_id,comment_id) VALUES(%s,%s)", (uid, cid)); liked = True
    cnt = c.execute("SELECT COUNT(*) cnt FROM comment_likes WHERE comment_id=%s", (cid,)).fetchone()["cnt"]
    c.commit(); c.close(); return safe_json({"liked": liked, "count": cnt})
...
def follow(b: FollowReq):
    ...
    if not tg: c.close(); return err("Topilmadi!", 404)
    if tg["id"] == fid: c.close(); return err("O'zingizga follow bosolmaysiz!")
    if c.execute("SELECT 1 FROM follows WHERE follower_id=%s AND following_id=%s", (fid, tg["id"])).fetchone():
        c.execute("DELETE FROM follows WHERE follower_id=%s AND following_id=%s", (fid, tg["id"])); f = False
    else:
        c.execute("INSERT INTO follows VALUES(%s,%s)", (fid, tg["id"])); f = True
    c.commit(); c.close(); return safe_json({"following": f})
...
def news_update(b: NewsEdit):
    ...
    c.execute("UPDATE school_news SET title=%s WHERE id=%s", (b.title.strip(), int(b.news_id)))
    c.commit(); c.close(); return safe_json({"success": True})
...
def news(user_id: Optional[Union[int, str]] = None):
    ...
    c.close(); return safe_json([dict(r) for r in rows])
...
def news_like(b: NewsLike):
    ...
    if c.execute("SELECT 1 FROM news_likes WHERE user_id=%s AND news_id=%s", (uid, nid)).fetchone():
        c.execute("DELETE FROM news_likes WHERE user_id=%s AND news_id=%s", (uid, nid)); lk = False
    else:
        c.execute("INSERT INTO news_likes(user_id,news_id) VALUES(%s,%s)", (uid, nid)); lk = True
    c.commit(); c.close(); return safe_json({"liked": lk})
...
def news_comment_like(b: NewsCommentLikeReq):
    ...
    if c.execute("SELECT 1 FROM news_comment_likes WHERE user_id=%s AND comment_id=%s", (uid, cid)).fetchone():
        c.execute("DELETE FROM news_comment_likes WHERE user_id=%s AND comment_id=%s", (uid, cid)); liked = False
    else:
        c.execute("INSERT INTO news_comment_likes(user_id,comment_id) VALUES(%s,%s)", (uid, cid)); liked = True
...
def rights(b: RightsReq):
    ...
    c.commit(); c.close(); return safe_json({"granted": bool(nv)})
...
def admin_delete_user(b: DeleteUserReq):
    ...
    c.close(); return safe_json({"success": True})
...
def remove_follower(b: RemoveFollowerReq):
    ...
    c.execute("DELETE FROM follows WHERE follower_id=%s AND following_id=%s", (fu["id"], int(b.owner_id)))
    c.commit(); c.close(); return safe_json({"success": True})
